...
N_SAVE_CHECKPOINT = 5
# File names
FILE_NAME_INVALID_URLS = "./data/No valid urls.txt"
FILE_NAME_NOT_ARTICLE_URLS = "./data/Not articles urls.txt"
# Dates and times
...
def extract_data_from_jsons(html, 
    ...
    for json_data_str in jsons:
        ...
        for k, _ in json_data.items():
            ...
            if not keys_found.get("type", False) and "@type" in k:
                if isinstance(json_data["@type"], list):
                    for value in json_data["@type"]:
                        ...
                        if (type_value.startswith("news") or type_value.endswith("article")):
                            type_key_found = True
                            break
                elif isinstance(json_data["@type"], str):
...
def extract_data_from_metadata(parsed_html, data, keys_found):
    ...
    for tag in meta_data:
        ...
        attribute_content = tag.attrs["content"]
...
def find_valid_news_urls(news_urls, 
                         media_url) -> (list, bool):
    valid_news_urls = []
    with open(FILE_NAME_INVALID_URLS, "w") as file_invalid:
        for news_url in news_urls:
            ...
            if news_url == media_url:
                print(f"{news_url} skipped. find_valid_news_urls();continue_1", file=file_invalid)
                continue
            if  news_url.endswith(".xml"):
                print(f"{news_url} skipped. find_valid_news_urls();continue_2", file=file_invalid)
                continue
            if "pagina-1.html" in news_url or "/firmas" in  news_url:
                print(f"{news_url} skipped. find_valid_news_urls();continue_3", file=file_invalid)
                continue
            if "/autor" in news_url or "/autores/" in  news_url:
                print(f"{news_url} skipped. find_valid_news_urls();continue_4", file=file_invalid)
                continue
            if media_url not in news_url:
                print(f"{news_url} skipped. find_valid_news_urls();continue_5", file=file_invalid)
                continue
            news_url_splits = [x for x in news_url.split("/") if x][2:]
            if len(news_url_splits) <= 1:
                print(f"{news_url};find_valid_news_urls();continue_6", file=file_invalid)
                continue
            elif len(news_url_splits) == 2:
                if re.search(r"^[.a-zA-Z0-9]+(-[.a-zA-Z0-9]+)?$", news_url_splits[-1]):
                    print(f"{news_url} skipped. find_valid_news_urls();continue_7", file=file_invalid)
                    continue
...
def find_news_data(news_urls, author, order_keys=True):
    news_media_data = []
    n_no_articlebody_in_article = 0
    file_no_articles = open(FILE_NAME_NOT_ARTICLE_URLS, "w")
    for news_url in news_urls:
        ...
        try:
            ...
            if keys_found is None:
                print(news_urls, "is not a valid Article")
                continue
            data.update(extracted_data)
            data, keys_found = extract_data_from_metadata(parsed_news_hmtl, data, keys_found)
            if not keys_found["title"]:
                continue
        ...
        if data is None or (not keys_found["title"] or not keys_found["body"]):
            continue
        # Add other keys
...
def split_file_and_process(sections_file_path, num_splits, process_function):
    ...
    for i, chunk in enumerate(chunks):
        ...
        process = multiprocessing.Process(target=process_function, args=(chunk, str(i)))
...
def main_multi_threading_process(sections_chunk, pid: str):
    ...
    n_processed = 0
    n_no_body = 0
    for i, section_url in enumerate(sections_chunk): 
        try:
        ...
        #section_url = "https://www." + section_url
...
if __name__ == "__main__":
    current_date, current_time = update_date(current_date, 
                                             current_time)
